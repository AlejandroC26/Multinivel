...
class UserController extends Controller
{
    public function __construct()
    {
        ...
        $this->middleware('us.permission:users.show', ['except'=> ['register', 'update', 'reset_password', ]]);
    ...
    public function register(Request $request)
    {
        ...
        /* ====VALIDATION==== */
        if(!$city_id) return response()->json(['status'=> 'error', 'message'=> 'City not found']);
        if(!$sponsor_user) return response()->json(['status'=> 'error', 'message'=> 'Sponsor user not found']);
        if (strpos($request->password, " ")) return response()->json(['status'=> 'error', 'message' => 'Password cannot contain spaces'], 400);
    ...
    public function showFile(Request $request, $id, $file) {
        ...
        switch ($request->file) {
            case 'consignment':
                $file_type = "user-consignment"; 
                $file_name = $user->consignment;
                break;
            
            default:
                return response()->json(['status'=> 'error', 'message'=> 'File type is not allowed'], 400);
                break;
        }
        if(!$file_name) return response()->json(['status'=> 'error', 'message'=> 'File doesnt found'], 400);
        $url = $file_type.'/'.$file_name;
    ...
    public function usersLine(Request $request, $user_id, $line_id){
        ...
        else if($line_id <= 0 or $line_id > 8) return response()->json(['status'=> 'error', 'message'=> 'Invalid line']);
