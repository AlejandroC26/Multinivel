            ...
            <!--------NAV TABS--------->
                <nav>
                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                        ...
                        <button class="nav-link" id="nav-devolution-tab" data-toggle="tab" data-target="#nav-devolution" type="button" role="tab" aria-controls="nav-devolution" aria-selected="false">Comprobante de Devolución</button>
                    </div>
                </nav>
                <div class="tab-content" id="nav-tabContent">
                        ...
                        <div class="row mt-2 mb-1">
                            <div class="col-md-6">
                                ...
                                <select class="form-control" v-model="user.count_type">
                                    ...
                                    <option value="Daviplata">Nequi</option>
                    ...
                    <!-- COMPROBANTES -->
                    <div class="tab-pane fade" id="nav-devolution" role="tabpanel" aria-labelledby="nav-devolution-tab">
                        <div class="row mt-3">
                            <div class="col-lg-12">
                                <div v-if="!session_user.state === 'Inactivo'">
                                    <label class="circle" for="file-consignment">+</label>
                                    <label for="file-consignment" class="label-link">Adjuntar comprobante</label>
                                </div>
                                <div v-else>
                                    <div class="mb-2" 
                                        v-for="consignment in user.consignments" :key="consignment.id">
                                        <p>{{consignment.type}}</p>
                                        <div class="edit-file-box">
                                            <span class="link" @click="downloadFile(consignment.id)">{{consignment.image}}</span>
                                            <span style="color: var(--color-dark-variant)">{{consignment.state}}</span>
                                            <label 
                                                @click="loadConsignmentId(consignment.id)"
                                                for="update-file-consignment" 
                                                class="material-icons-sharp icon-edit" 
                                                v-if="consignment.state === 'Espera' || consignment.state === 'Rechazado'"
                                            >&#xe3c9;</label>
                                        </div>
                                    </div>
                                    <div 
                                        v-if="(user.consignments.length < 3)"
                                    >
                                        <p v-if="(user.consignments.length === 0)">Activación</p>
                                        <div v-if="(session_user.state == 'Revisión')">
                                            <p>Espera hasta ser activado para iniciar con el ahorro</p>
                                        </div>
                                        <div v-else>
                                            <p v-if="(user.consignments.length >= 1)">Devolución {{user.consignments.length}}</p>
                                            <label class="circle" for="file-consignment">+</label>
                                            <label for="file-consignment" class="label-link">Adjuntar comprobante</label>
                                        </div>
                                    </div>
                                ...
                                <input 
                                    class="hidden-file" 
                                    type="file" 
                                    id="update-file-consignment" 
                                    accept="image/.png,.jpeg,.jpg"
                                    @change="updateFileConsignment()"
                                >
                            </div>
                        </div>
                    </div>
                ...
                <div class="row mt-3">
                    ...
                    <div class="col-md-3">
                        <button class="btn btn-primary" style="width: 100%;" @click="updateUserFunction(user)">Guardar</button>
                    </div>
                </div>
            </div>
            <!--------LOADING-------->
...
export default {
    ...
    methods: {
        loadConsignmentId(id){this.id_consign = id},
        getUserConsignments(id){
            let url = this.api_url+`/api/users/${id}/listUserConsignments`;
            axios.get(url,
            { headers: { "Authorization": "Bearer " + localStorage.getItem('token')}})
            .then(res => this.user.consignments = res.data)
        ...
        async updateFileConsignment(){
            let file = document.getElementById('update-file-consignment').files[0];
            let form = new FormData();
            form.append('image', file);
            let url = this.api_url+`/api/users/${this.id_consign}/updateConsignmentFile`;
            axios.post(url, form,
            { headers: { "Authorization": "Bearer " + localStorage.getItem('token')}})
            .then(res => {
                if(res.status === 201) {
                    this.getUserConsignments(this.session_user.id);
                    this.loadSessionUser({reload: true});
        ...
        async downloadFile(id){
            try {
                const imgFile = await this.showFile({id});
